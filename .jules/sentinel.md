...
## 2025-05-18 - Eliminating Command Injection in Clipboard Copying via `execFileSync`

**Vulnerability:** `src/cli/copy.ts` previously executed system clipboard helper commands (e.g. `xclip -selection clipboard`) using `execSync(program)`. Invoking sub-processes via shell string evaluation allows command injection risks if parameters or environment variables are manipulated.
**Learning:** System commands with arguments should be passed as discrete string arrays to execution functions like `execFileSync` to avoid subshell spawning and shell parsing.
**Prevention:** Always use `execFileSync(binary, argsArray, options)` instead of `execSync(string)` when executing system binaries with flags.
