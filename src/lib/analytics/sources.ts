...
// Run command using execFile to prevent shell command injection
function runCommand(file: string, args: string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    execFile(file, args, (error, stdout, stderr) => {
      if (error) {
        debug('Error trying to check command installation', error);
        return reject(error);
      }
...
export async function isInstalled(commandToCheck: string): Promise<boolean> {
  ...
  let whichArgs: string[] = [commandToCheck];
